...
async def _create_test_fix_branch(repo_path: str, branch_name: str, fixes: list[dict]) -> tuple[bool, str]:
    ...
    try:
        ...
        subprocess.run(
            ["git", "-C", repo_path, "commit", "-m", "fix(qa): update selectors and generated tests"],
            check=True,
            timeout=10,
        )
...
async def create_test_fix_pr(repo_url: str, test_output: str, fixes: str, workspace_dir: Path) -> str:
    ...
    branch_name = f"ai-qa/autonomous-qa-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    pr_title = "feat(qa): autonomous council test generation and healing updates"
    pr_body = f"""## Summary of Coverage
- Added/updated generated artifacts: {len(fix_list)}
- Included layers: unit/integration/e2e/page-object scaffolding

## Repair Logs
- Healer processed failure payloads from Executor output
- Selector patches applied where matching page object selectors were found

## Test Results
| Metric | Value |
|---|---|
| Exit Snapshot | {('captured' if test_output else 'not available')} |
| Failure Payloads | {test_output.count('"error"')} |
| Generated Files | {len(fix_list)} |

## Detailed Notes
{test_output[:1200] if test_output else 'Test analysis completed'}
"""
